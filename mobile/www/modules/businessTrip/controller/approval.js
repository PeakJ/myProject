/**
 * Created by 11150421040204 on 2015/7/30.
 */
/**
 * 待我审核
 * 韩旭
 */
bomApp.controller("businessTripApprovalCtrl", ['$scope', '$stateParams', 'myApprovalService', '$ionicPopup',
    'checkResultServices', 'userService', '$ionicHistory', '$ionicViewSwitcher', 'loadingUtil', 'photoService', 'checkUtil', 'UrlService',
    function ($scope, $stateParams, myApprovalService, $ionicPopup, checkResultServices,
              userService, $ionicHistory, $ionicViewSwitcher, loadingUtil, photoService, checkUtil, UrlService) {
        ...
        $scope.judge = true;
        loadingUtil.show();
        myApprovalService.get(urlParameter).then(function (data) {
            console.log(data);
            /*显示从后台传过来的数据*/
            if (data.length != 0) {
                var trafficIds = '';
                ...
                for (var i = 0; i < data.tripApplyChecks.length; i++) {
                    ...
                    if (data.tripApplyChecks[i].userLevel == '4') {
                        if (data.tripApplyChecks[i].userId == _userId && data.tripApplyChecks[i].checkStatus != 0) {
                            $scope.judge = false;
                        }
