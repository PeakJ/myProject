 ...
 */
cdfgApp.controller('AddressDetailController', ['$scope', '$ionicHistory', '$timeout',
...
cdfgApp.controller('AddressDetailFormController', ['$scope', '$stateParams', '$rootScope', 'AddressService',
    'LocationService', '$ionicHistory', 'PopupService',
    function ($scope, $stateParams, $rootScope, AddressService, LocationService, $ionicHistory, PopupService) {
        ...
        //初始化
        $scope.initFlag = $stateParams.init;
        $scope.init = function () {
        ...
        $scope.submitAddress = function () {
            console.log('ready to submit addr!');
            formatGeoName($scope.addressInfo);
            $scope.validate = true;
            if ($scope.addressDetailForm.$valid &&
                ($scope.addressInfo.province || $scope.addressInfo.city || $scope.addressInfo.county || $scope.addressInfo.town)) {
                ...
                PopupService.promptPopup('详细地址填为5～50个字符', 'error');
